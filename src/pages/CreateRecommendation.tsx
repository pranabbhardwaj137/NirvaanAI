...
function CreateRecommendation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error.response?.data?.error || 'Failed to create recommendation');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-red-500 text-xl">Please log in to create a recommendation</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-stress-dark py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-3xl font-bold text-white mb-8">Create a Recommendation</h1>
